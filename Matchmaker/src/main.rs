...
use axum :: {
    ...
    routing :: post,
    Json as AxumJson , Router , http :: {StatusCode , HeaderMap},
...
async fn AuthMiddleware(
    ...
    let AuthHeader : &str = Headers.get("X-Api-Key")
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let mut Connection : MultiplexedConnection = match LoopState.RedisClient.get_multiplexed_async_connection().await {
                Ok(C) => C,
                Err(E) => { error!("Redis Connection Error: {}" , E); continue; }
            };
            ...
            let mut Tickets : Vec <MatchmakingTicket > = RawTickets.iter()
                .filter_map(|S| serde_json::from_str::<MatchmakingTicket >(S).ok())
                .collect();
            ...
            for FoundMatch in Matches {
                ...
                let Members : Vec <PartyMember > = FoundMatch.iter().flat_map(|T| T.Members.clone()).collect();
                let MatchedTicketIds : Vec <String > = FoundMatch.iter().map(|T| T.TicketId.to_string()).collect();
                
                let InternalState : Arc <AppState > = LoopState.clone();
                tokio::spawn(async move {
                    if let Err(E) = SendMatchToRobloxWithRetry(MatchId , Members , InternalState.clone()).await {
                        error!("FATAL: Match notification failed: {}" , E);
                    } else {
...
async fn SubmitTicket(
    ...
    let mut Connection : MultiplexedConnection = Data.RedisClient.get_multiplexed_async_connection().await
...
async fn SendMatchToRobloxWithRetry(
    ...
    let Url : String = format!("https://apis.roblox.com/messaging-service/v1/universes/{}/topics/{}" , Config.RobloxUniverseId , Config.RobloxTopic);

    let MessageBody : String = json!({ "MatchId": MatchId, "PlayerIds": PlayerIds }).to_string();
    ...
    loop {
        ...
        match Response {
            ...
            Err(E) => warn!("Attempt {} failed (Error): {}" , CurrentAttempt , E),
        }
